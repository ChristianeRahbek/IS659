...
public:
    ...
    void doAnalysis() {
        ...
        /** We only want hits with multiplicity of 2, as we do not have to worry about coincidences
         *  between two DSSDs AND with a Plastic. This would be ideal to have that functionality,
         *  but I am not capable of doing so as of now...
         * */
        if (!(mult == 2)) return;
    ...
    unique_ptr<DynamicBranchVector<double>> v_Edep0, v_Edep1;
...
void prepareFileIO(const string &configfile) {
    ...
    if (cfg.lookup("paths_relative_to_project_root")) {
        ...
        output_dir = EUtil::getProjectRoot() + "/" + cfg.lookup("data_output_dir_2coincidence").c_str();
    } else {
        setup_path = cfg.lookup("setup_file").c_str();
        target_path = cfg.lookup("target_file").c_str();
        input_path = cfg.lookup("data_input").c_str();
        output_dir = cfg.lookup("data_output_dir_2coincidence").c_str();
    }
